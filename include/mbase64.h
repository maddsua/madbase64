...
#ifndef maddsuabase64
#define maddsuabase64
	#include <string.h>
	#include <stdint.h>
	#include <stdbool.h>
	#include <malloc.h>
...
//	prototypes
/*
	bool mad_b64Valid(const char* basedString);
	char* mad_b64Enc(const uint8_t* source, size_t sourceLen);
	uint8_t* mad_b64Dec(const char* baseString, size_t* decodedLen);
*/
...
char* mad_b64Enc(const uint8_t* source, size_t sourceLen) {
	...
	const char base64_EncodeTable[] = "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/";
	
	//	safe margins and other good stuff to not to shoot yourself in the foot
	const short safeMargin = 8;
	const size_t plainSize = ((sourceLen + safeMargin) * sizeof(uint8_t));
	...
	/*
		And now we deal with lenght and all of that bs... no I'm just kidding, we put '=' and a terminator at the end of the asci string, aka base64-encoded string, and thats all.
	
		It works cause we're returning a c string that is at least 5 bytes longer that its actual content. So when you call strlen() on it or copy it, it will only "see" a valid base64 string and none of tail-garbage
	 */
...
uint8_t* mad_b64Dec(const char* baseString, size_t* decodedLen) {
	...
	const uint8_t base64_DecodeTable[256] = {
	...
	const size_t decdSize = ((decdLen + safeMargin) * sizeof(uint8_t));
	uint8_t* decoded = (uint8_t*)malloc(decdSize);
